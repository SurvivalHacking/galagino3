 ...
 *
 * V3.2A 21/06/2026
 * Added support for alternate Cheap Yellow Display ESP32-024, need to move Joystick Connector to upper right connector integrating +3v3 wire.
 *
...
Menu menu = Menu();
  ...
  Led led = Led();
#endif
SSD1306Wire flyer(0x3c, SDA_PIN, SCL_PIN); // flyer logo support www.survivalhacking.it
//#ifdef USE_I2C_INPUT
...
void setup() {
  ...
  if (pcfFound) {
    ...
    pcf20.begin(SDA_PIN, SCL_PIN, 255);
    pcf20.write8(0xFF);
  }
  #endif
...
void showQRonTFT() {
  ...
  for (int py = 0; py < 320; py++) {
    ...
    // Testo "GALAGINO V3.2A"
    else if (py >= textY + charH + 4 && py < textY + charH * 2 + 4) {
      int charRow = py - (textY + charH + 4);
      int textW = gfxStringWidth("GALAGINO V3.2A", font);
      int textX = (240 - textW) / 2;
      tft_drawStringRowGFX(rowBuf, textX, "GALAGINO V3.2A", charRow,
...
void showBootScreen() {
  ...
  oled_drawStringGFXCenter(flyer, 64, 36, "V3.2A BY", &FreeSans12pt7b);
  flyer.display();
